...
//		System.out.println(message);
		if(lastTimestamp > 0 && lastTimestamp < curTime - timeRange) {
			...
			long tmp;
			//getNext moves the cursor to the next key/data pair and returns that pair
			while(cursor.getNext(key, data, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
				tmp = LongBinding.entryToLong(key); //timestamp of expired triples				
				if(tmp < (curTime - timeRange)) {
					cursor.delete();
//					System.out.println("purge " +lastTimestamp + " cur "+curTime+ " "+ "w "+ w + " curTime-w "+(curTime-w));
				}
				else {
					cursor.close();
					lastTimestamp = tmp;
